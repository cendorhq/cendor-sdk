...
Yes. `budget`, `track`, `Policy`, `AuditLog`, and `trace` re-exported from `cendor.sdk` **are**
the [tokenguard](/docs/tokenguard)/[acttrace](/docs/acttrace) originals, and `guard` is acttrace's
policy enforcement in the SDK's scope form — so a `budget()` opened
