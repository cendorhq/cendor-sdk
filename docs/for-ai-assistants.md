...
| SDK guardrail gates | `Agent(reask_on_output_trip=2, stream_check_window=200)` | `new Agent({ reaskOnOutputTrip: 2, streamCheckWindow: 200 })` | Bounded re-ask on an output block (**non-streaming**) + streaming output-window checks — **both languages** since SDK 1.15 / 0.20 (camelCase in TS). Streaming re-ask is offered in neither (streamed deltas can't be unshown). |
